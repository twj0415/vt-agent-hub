...
<template>
  <div class="overflow-hidden rounded-vt-md border border-line bg-surface-2/40">
    <article
      v-for="item in storagePathCards"
      :key="item.key"
      class="grid gap-2 border-b border-line/55 px-4 py-4 last:border-b-0 xl:grid-cols-[180px_minmax(0,1fr)]"
    >
      <div>
        <div class="text-[13px] font-semibold text-text">{{ pathLabel(item.key) }}</div>
        <div class="mt-1 text-[11px] text-muted">{{ item.key }}</div>
      </div>
      <div class="min-w-0">
        <div class="vt-mono break-all rounded-vt-sm border border-line bg-panel/70 px-3 py-2 text-[12px] text-text">{{ item.path }}</div>
        <div class="mt-2 text-[13px] leading-6 text-muted">{{ item.note }}</div>
      </div>
    </article>
  </div>
</template>
