...
<template>
  ...
  <RepositoryImportModal
    ...
    :local-preview="skillStore.localPreview"
    :local-import-result="skillStore.localImportResult"
    :local-selections="skillStore.localSelections"
    :local-selected-path="skillStore.localSelectedPath"
    :local-step="skillStore.localStep"
    @close="skillStore.setImportOpen(false)"
    @preview="skillStore.previewSkillImport()"
    @apply="skillStore.applySkillImport()"
    @field="skillStore.setImportField($event.key, $event.value)"
    @source-kind="skillStore.setSourceKind($event)"
    @pick-local="skillStore.pickLocalSkillsPath()"
    ...
    @toggle-local-skill="skillStore.toggleLocalSkillSelection($event)"
    @local-resolution="skillStore.setLocalSkillResolution($event.sourcePath, $event.resolution)"
    @local-rename="skillStore.setLocalSkillRename($event.sourcePath, $event.renamedSkillId)"
    @local-selected-path="skillStore.setLocalSelectedPath($event)"
  />
</template>
